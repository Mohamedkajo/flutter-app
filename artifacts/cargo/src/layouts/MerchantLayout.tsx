...
export function MerchantLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { label: 'Store Settings', path: '/merchant/store', icon: Settings },
  ];

  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-sidebar">
      <div className="p-6">
        <h2 className="text-2xl font-bold text-primary">Cargo<span className="text-accent">Merchant</span></h2>
      </div>
      ...
      <div className="p-4 border-t border-border">
        <div className="flex items-center gap-3 mb-4 px-2">
          <Avatar className="h-10 w-10">
            ...
            <AvatarFallback className="bg-primary/10 text-primary">{user?.name?.charAt(0) || 'M'}</AvatarFallback>
          </Avatar>
    ...
    <div className="min-h-[100dvh] bg-surface flex flex-col md:flex-row">
      ...
      <header className="md:hidden sticky top-0 z-50 bg-white border-b border-border h-16 flex items-center px-4 justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-primary">Cargo<span className="text-accent">Merchant</span></h2>
        </div>
      </header>
